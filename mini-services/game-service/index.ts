...
// Content is now generated DIRECTLY in the game service using z-ai-web-dev-sdk
  ...
  for (let attempt = 1; attempt <= MAX_CONTENT_RETRIES; attempt++) {
    try {
      ...
      try {
        ...
        const searchPromise = zai.functions.invoke('web_search', { query: randomQuery, num: 10 })
        ...
        if (Array.isArray(searchResults) && searchResults.length > 0) {
          const goodResults = searchResults.filter((r: any) => r.snippet && r.snippet.length > 50)
          const chosenResult = goodResults.length > 0
      ...
      if (!completion) {
        console.error(`[fetchGameContent] Attempt ${attempt}: LLM timed out`)
        continue
      }

      const responseText = completion?.choices?.[0]?.message?.content || completion?.content || ''
      if (!responseText || typeof responseText !== 'string') {
        console.error(`[fetchGameContent] Attempt ${attempt}: Empty LLM response`)
        continue
      }
