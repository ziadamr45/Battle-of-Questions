...
- الخيارات متقاربة جداً: كلها تبدو صحيحة لمن لم يتذوق النص بعمق
- ممنوع أسئلة الحل المباشر: الإجابة لازم تتطلب تذوقاً أدبياً وتحليلاً بلاغياً
...
- مثال: "أي القولين أبلغ في التعبير عن... ولماذا؟"`,
...
- كل فقرة يجب أن تضيف بُعداً جديداً للنص بحيث لا يمكن فهم الفقرة الأخيرة بدون فهم الأولى`,
  ...
  صعب: `مستوى صعب جداً (متقدم):
- نص أدبي شديد التركيب والتعقيد: طبقات بلاغية متداخلة ومتراكبة، رموز فوق رموز فوق رموز، كل صورة تحمل أكثر من تأويلين وكل إيحاء يخفي إيحاء آخر
- أسئلة نقدية وتحليلية عميقة جداً ومتعددة المستويات: تتطلب تفكيك النص بالكامل وربط كل أجزائه البعيدة واكتشاف العلاقات الخفية بين الطبقات المختلفة
- الخيارات مصممة ببراعة فائقة: كل خيار يحمل جزءاً من الصحة البلاغية لكن واحد فقط يلتقط المعنى الكامل بدقة. الفروقات شبه معدومة ظاهرياً وتحتاج تذوقاً بلاغياً وفكرياً حاداً
- ممنوع تماماً أي سؤال مباشر أو متوقع — كل سؤال يجب أن يفاجئ حتى القارئ الخبير ويجبره على إعادة قراءة النص بزاوية جديدة
- النص يجب أن يكون متاهة أدبية مركبة مزدوجة: رموز مركبة ثلاثية الدلالة، إيحاءات متضادة ومتكاملة في نفس الوقت، صور تُقرأ على أكثر من مستويين
- استخدم التورية المركبة المزدوجة والاستعارة التضمينية المتراكبة والترميز الدائري المتعدد حيث يعود المعنى لبدايته بأسلوب جديد في كل دورة
- كل فقرة يجب أن تضيف بُعداً جديداً للنص وتعيد تعريف ما سبقها بحيث لا يمكن فهم الفقرة الأخيرة بدون فهم الأولى ولا يمكن فهم الأولى بدون الفقرة الأخيرة
- النص يجب أن يضع القارئ في معركة نقدية حقيقية: لا يكفي أن يقرأ بل يجب أن يُعيد قراءة كل جملة مرتين ويربطها بما سبق وما يلي
- اجعل بعض الأسئلة تتطلب فهم النص بأكمله وفهم العلاقات بين كل أجزائه قبل الإجابة`,
}
...
أجب بصيغة JSON فقط:
{
  ...
  const wordCounts: Record<Difficulty, string> = {
    سهل: '400-600 كلمة على الأقل',
    متوسط: '550-800 كلمة على الأقل',
    صعب: '750-1100 كلمة على الأقل',
  }
  ...
  const difficultyInstructions: Record<Difficulty, string> = {
    سهل: `مستوى متوسط (سابقًا كان سهل):
- أسئلة قدرات عليا (تحليل - تركيب - تقويم - استنتاج بعيد): الإجابة مش مباشرة خالص واحتياج تفكير عميق
- الخيارات متقاربة جداً: كل الخيارات تبدو صحيحة ومانعرفش الفرق بينها إلا لو فاهم النص كويس
- ممنوع تماماً أي سؤال مباشر من النص: كل الأسئلة لازم تتطلب استنتاج وتحليل
...
- النص لازم يخلّي القارئ يفكر ويرجع يقرأه تاني عشان يكتشف طبقات جديدة
- مثال: "ما الموقف الفلسفي الضمني الذي يتبناه الكاتب دون التصريح به؟"`,
...
- كل فقرة يجب أن تكون مرتبطة بالفقرة اللي قبلها بعلاقة غير مباشرة والقارئ لازم يكتشفها بنفسه`,
    ...
    صعب: `مستوى صعب جداً (متقدم):
- أسئلة قدرات إبداعية ونقدية عليا (تركيب مركب مزدوج - تقويم نقدي مقارن - استنتاج بعيد متعدد المراحل والمستويات): الإجابة تتطلب تفكيك النص بالكامل وربط كل أجزائه البعيدة واكتشاف العلاقات الخفية بين الطبقات المختلفة
- الخيارات مصممة ببراعة فائقة: كل خيار يحمل جزءاً من الصحة لكن واحد فقط يلتقط المعنى الكامل بدقة. الفروقات شبه معدومة ظاهرياً وتحتاج تذوقاً لغوياً وفكرياً حاداً
- ممنوع تماماً أي سؤال مباشر أو شبه مباشر أو متوقع — كل سؤال يجب أن يفاجئ حتى القارئ الخبير ويجبره على إعادة قراءة النص بزاوية جديدة
- النص لازم يكون مكتوب بأسلوب فكري فلسفي شديد التعقيد والتراكب: مفاهيم متداخلة مزدوجة وثلاثية، مفارقات ظاهرية تخفي حقائق مختلفة كلها صحيحة في مستويات مختلفة
- اكتب كأنك تكتب رسالة دكتوراه في الفلسفة والأدب: حبكة فكرية شديدة التعقيد، مفاهيم متراكبة ومتداخلة، كل فقرة تضيف بُعداً جديداً وتعيد تعريف ما سبقها
- النص لازم يكون متاهة فكرية وأدبية مركبة: القارئ لازم يرجع يقرأه أكتر من مرتين عشان يكتشف كل طبقة وكل رمز وكل علاقة خفية
- استخدم الرمز المركب المزدوج (رمز داخل رمز يحمل معنى ثالث) والتضاد المتعدد المستويات والمفارقة العميقة المتكررة والتمثيل الفلسفي الدائري
- كل فقرة يجب أن تكون مرتبطة بالفقرة اللي قبلها وبعدة فقرات أخرى بعلاقات غير مباشرة وغير متوقعة والقارئ لازم يكتشفها بنفسه
- اجعل بعض الأسئلة تتطلب فهم النص بأكمله قبل الإجابة — لا يمكن الإجابة على أي سؤال بشكل مستقل`,
  }
...
7. أضف شرحاً لماذا الإجابة الصحيحة هي الصحيحة

8. التزام تام بقواعد اللغة العربية

9. جودة النص أولوية قصوى:
  ...
  for (let attempt = 1; attempt <= MAX_CONTENT_RETRIES; attempt++) {
    try {
      ...
      const systemMessage = gameType === 'نصوص'
        ...
        : 'أنت معلم خبير في اللغة العربية متخصص في إعداد امتحانات القراءة المتحررة والنصوص. تُجيب دائماً بصيغة JSON صالحة فقط بدون أي نص إضافي.'
