...
5. ${nosousDifficultyInstructions[difficulty]}

6. أضف شرحاً بلاغياً لماذا الإجابة الصحيحة هي الصحيحة — اشرح الصورة البيانية أو المحسن البديعي أو الأسلوب

7. التزام تام بقواعد اللغة العربية والبلاغة

8. جودة النص الأدبي أولوية قصوى:
...
أجب بصيغة JSON فقط:
{
  ...
  const difficultyInstructions: Record<Difficulty, string> = {
    سهل: 'أسئلة فهم واستنتاج: الإجابة مش موجودة نصاً في النص لكن لازم تفهم وتستنتج. الخيارات متقاربة. ممنوع أسئلة الحل المباشر: الإجابة لازم تتطلب تفكير واستنتاج مش مجرد نسخ من النص. النص لازم يكون مكتوب بأسلوب أدبي عميق مع طبقات معنوية متعددة — استخدم لغة ثرية وصور بيانية وحجج مترابطة. اكتب بأسلوب يشبه المقالات الفكرية والأدبية العميقة: أفكار مترابطة، انتقالات سلسة، استنتاجات منطقية.',
    متوسط: 'أسئلة قدرات عليا (تحليل - تركيب - تقويم - استنتاج بعيد): الإجابة مش مباشرة خالص واحتياج تفكير عميق. الخيارات متقاربة جداً جداً. ممنوع تماماً أي سؤال مباشر من النص. النص لازم يكون مكتوب بأسلوب أدبي فكري مركّب وكثيف — استخدم تراكب المفاهيم والتناقضات الظاهرية والعمق الفلسفي. اكتب كأنك تكتب مقالاً فكرياً مرموقاً: حبكة فكرية معقدة، مفاهيم متداخلة، مفارقات، تساؤلات مفتوحة. النص لازم يخلّي القارئ يفكر مرتين ويرجع يقرأه تاني عشان يكتشف طبقات جديدة. استخدم التضاد والمفارقة والرمز والتمثيل بشكل مكثّف.',
    صعب: 'أسئلة قدرات عليا متقدمة (تركيب مركب - تقويم نقدي - استنتاج بعيد متعدد المراحل): الإجابة تتطلب تفكيك النص كله وربط أجزائه البعيدة. الخيارات متطابقة تقريباً في الظاهر: الفرق بينها دقيق للغاية ويحتاج تحليلاً عميقاً ودقيقاً. ممنوع تماماً أي سؤال مباشر أو شبه مباشر — كل الأسئلة يجب أن تكون غير متوقعة. النص لازم يكون مكتوب بأسلوب فكري فلسفي شديد التعقيد: مفاهيم متداخلة مزدوجة، مفارقات ظاهرية تخفي حقيقة مختلفة. اكتب كأنك تكتب رسالة دكتوراه في الفلسفة: حبكة فكرية شديدة التعقيد، مفاهيم متراكبة، كل فقرة تضيف بُعداً جديداً. النص لازم يكون متاهة فكرية: القارئ لازم يرجع يقرأه أكتر من مرة عشان يكتشف كل طبقة. استخدم الرمز المركب (رمز داخل رمز) والتضاد المتعدد والمفارقة العميقة والتمثيل الفلسفي. كل فقرة يجب أن تكون مرتبطة بالفقرة اللي قبلها بعلاقة غير مباشرة والقارئ لازم يكتشفها بنفسه.',
  }
